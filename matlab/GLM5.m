...
for ss=1:size(data,2)
    ...
    if include(ss)==1
            ...
            for v=1:13
                count_opt1_chosen.(blkname{i})(is,v)=sum(opt1_out(blkidx)==v&chosen(blkidx));
                count_opt1_unchosen.(blkname{i})(is,v)=sum(opt1_out(blkidx)==v&~chosen(blkidx));
                count_opt2_chosen.(blkname{i})(is,v)=sum(opt2_out(blkidx)==v&~chosen(blkidx));
                count_opt2_unchosen.(blkname{i})(is,v)=sum(opt2_out(blkidx)==v&chosen(blkidx));
            end
            end
    end
end
 for i=1:length(blkname) 
     figure;
            subplot(2,2,1); imagesc(count_opt1_chosen.(blkname{i}));title([(blkname{i}),': opt1 chosen'])
         subplot(2,2,2);imagesc(count_opt1_unchosen.(blkname{i}));title([(blkname{i}),': opt1 unchosen'])
            subplot(2,2,3);imagesc(count_opt2_chosen.(blkname{i}));title([(blkname{i}),': opt2 chosen'])
            subplot(2,2,4);imagesc(count_opt2_unchosen.(blkname{i}));title([(blkname{i}),': opt2 unchosen'])
 end

function [out]=fit_linked_PNPE(opt1_events,opt2_events,choice,start,abandontn,resp_made,alphabins,betabins,fig_yes)
...
for k=1:length(i)
for j=1:length(i)
    learn_left=rescorla_wagner_PNPE_2lr(opt1_events,opt2_events,[inv_logit(i(k)) inv_logit(i(j))],start);
    val_l(k,j,:)=learn_left; 
end
end
...
beta=repmat(permute(exp(b_label),[1 3 4 2]),[length(i) length(i) size(opt1_events,1)*size(opt2_events,2) 1]);
...
% choice of a given participant
  ...
  ch=repmat(permute(chtmp,[3 4 1 2]),[length(i) length(i)  1 length(b_label)]);
...
out.marg_alpha_pos=squeeze(sum(sum(out.posterior_prob,3),2));
...
out.mean_alpha_pos=inv_logit(i*out.marg_alpha_pos);

% this calculates the variance of the distribution of learning rates

out.var_alpha_pos=inv_logit(((i-logit(out.mean_alpha_pos)).^2)*out.marg_alpha_pos);


out.marg_alpha_neg=squeeze(sum(sum(out.posterior_prob,1),3))';

% This generates the expected value of the learning rate using a weighted
% sum-- marginal probabilities multiplied by learning rate values. Note
% for both the learning rate and temperature mean and variance are
% caculated in log space
out.mean_alpha_neg=inv_logit(i*out.marg_alpha_neg);

% this calculates the variance of the distribution of learning rates

out.var_alpha_neg=inv_logit(((i-logit(out.mean_alpha_neg)).^2)*out.marg_alpha_neg);
...
if fig_yes==1 
   figure
   subplot(2,2,1);
   plot(alphalabel,out.marg_alpha_pos);
   title('Positive Learning Rate');
   subplot(2,2,2);
   plot(alphalabel,out.marg_alpha_neg);
...
bel=rescorla_wagner_PNPE_2lr(opt1_events,opt2_events,[out.mean_alpha_pos out.mean_alpha_neg],start);

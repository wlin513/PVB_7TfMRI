function [out]=fit_linked_PEIRS(opt1_events,opt2_events,choice,start,abandontn,resp_made,alphabins,betabins,omegabins,spreadbins,fig_yes)
...
b_log=log(betamin):(log(betamax)-log(betamin))/(betabins-1):log(betamax);

o=-10:(10-(-10))/(omegabins-1):10;
s_log=log(0.01):(log(1)-log(0.01))/(spreadbins-1):log(1);

for k=1:length(i)
for j=1:length(i)
    for l=1:length(s_log)
    [v1(k,j,l,:,:),v2(k,j,l,:,:),s1(k,j,l,:,:),s2(k,j,l,:,:)]=PEIRS(opt1_events,opt2_events,inv_logit(i(k)),inv_logit(i(j)),start,exp(s_log(l)));
    end
end
end
...
val_l=q1-q2;
mmdl=repmat(val_l,[1,1,1,1,1,1,length(b_log)]);

clear val_l q1 q2 

beta=repmat(permute(exp(b_log),[1 3 4 5 6 7 2]),[length(i) length(i) length(s_log) size(opt1_events,1) size(opt2_events,2) length(o) 1]);
...
% choice of a given participant
  ...
  ch=repmat(permute(choice,[3 4 5 1 2 6 7]),[length(i) length(i) length(s_log) 1 1 length(o) length(b_log)]);
  ...
  %renormalise
  out.posterior_prob=out.posterior_prob./(sum(sum(sum(sum(sum(out.posterior_prob))))));
...
out.marg_alphaQ=squeeze(sum(sum(sum(sum(out.posterior_prob,5),4),3),2));
...
out.marg_alphaS=squeeze(sum(sum(sum(sum(out.posterior_prob,1),3),4),5))';
...
out.marg_spread=squeeze(sum(sum(sum(sum(out.posterior_prob,1),2),4),5));
out.mean_spread=exp(s_log*out.marg_spread);
out.var_spread=exp(((s_log-log(out.mean_spread)).^2)*out.marg_spread);
    

out.marg_omega=squeeze(sum(sum(sum(sum(out.posterior_prob,1),2),3),5));
out.mean_omega=o*out.marg_omega;
out.var_omega=(o-out.mean_omega).^2*out.marg_omega;
%beta 

    out.marg_beta=squeeze(sum(sum(sum(sum(out.posterior_prob,1),2),3),4));
...
if fig_yes==1 
   ...
   plot(omegalabel,out.marg_omega);
    title('omega');
       subplot(2,3,5);
   plot(spreadlabel,out.marg_spread);
    title('spread');
   out.figure=f;
end
%
% get LL from estimated parameters
[v1,v2,s1,s2]=PEIRS(opt1_events,opt2_events,out.mean_alphaQ, out.mean_alphaS,start,out.mean_spread);
pes=(v1+v2)./2-0.5;
...
out.BIC=(2.*out.neg_log_like)+5*log(sum(sum(resp_made))); % -2ln(LL)+kln(n)
out.AIC=(2.*out.neg_log_like)+10;

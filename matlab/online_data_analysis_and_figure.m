...
for i=1:size(data,2)
     disp(i) 
      choices=transpose(reshape(data(i).choice,ntrials,length(data(i).blockN)));
       blk=find(data(i).blockN==9|data(i).blockN==11|data(i).blockN==13|data(i).blockN==15);
...
save('model_results_online_270624.mat',  '-v7.3')
%%
...
f5=figure;
set(gcf,'Position',[300 300 370 250])
models={'Bayesian_CVaR_eta_beta','rw','PNPE','PEIRS','concave_UTIL','convex_UTIL','inverse_s_shape_UTIL'};%,'Bayesian_CVaR_eta_priorvar','PEIRS'
models_names={'Bayesian-CVaR','1lr-RW','pos-neg-RW','PEIRS','concave-UTIL','convex-UTIL','inverse-s-shape-UTIL'};
BICs=[];
for i=1:length(models)
BICs=[BICs;sum(ee.(models{i}).AIC)];
end
barh(BICs-min(BICs),'FaceColor',colors(end,:),'EdgeColor',colors(end,:))
%title(datasetname,'FontSize',10)
ax=gca;
...
[r,p]=corr(beh_results.pro_variance_bias,model1.mean_eta_logit)
[r,p]=corr(beh_results.pro_variance_bias,model2.pos_bias_lrs_logit)

[r,p]=corr(ques_table.zscore_AD,model2.pos_bias_lrs)
[r,p]=corr(ques_table.zscore_AD,model2.mean_alpha_neg_logit)

[r,p]=corr(ques_table.zscore_AD,model1.mean_eta_logit)

[r,p]=corr(beh_results.pro_variance_bias,ques_table.zscore_AD)
%% color setting
...
exportgraphics(f4,[figdir,'online_corrplt_PVB_SDS.eps'],'BackgroundColor','none','ContentType','vector')
saveas(f4,[figdir,'online_corrplt_PVB_SDS.png'])

f4=figure;
set(gcf,'Position',[300 300 300 250])
plotcorr(beh_results.pro_variance_bias,ques_table.STAI,1,1,'pro-variance bias','STAI',col2);
exportgraphics(f4,[figdir,'online_corrplt_PVB_STAI.eps'],'BackgroundColor','none','ContentType','vector')
saveas(f4,[figdir,'online_corrplt_PVB_STAI.png'])

f4=figure;
set(gcf,'Position',[300 300 300 250])
plotcorr(PNPE_model.neg_bias_lrs_logit,ques_table.SDS,1,1,'negative bias in learning rates','SDS',col2);
exportgraphics(f4,[figdir,'online_corrplt_LR_SDS.eps'],'BackgroundColor','none','ContentType','vector')
saveas(f4,[figdir,'online_corrplt_LR_SDS.png'])

f4=figure;
set(gcf,'Position',[300 300 300 250])
plotcorr(PNPE_model.neg_bias_lrs_logit,ques_table.STAI,1,1,'negative bias in learning rates','STAI',col2);
exportgraphics(f4,[figdir,'online_corrplt_LR_STAI.eps'],'BackgroundColor','none','ContentType','vector')
saveas(f4,[figdir,'online_corrplt_LR_STAI.png'])
%% plot AIC
colors=[repmat([0.68,0.85,0.92],4,1);repmat([0.415,0.686,0.902],4,1)];
f4=figure;
set(gcf,'Position',[300 300 300 350])
models={'Bayesian_CVaR_eta_beta','PNPE','rw','PEIRS'};
models_names={'Bayesian-CVaR','2lr-RW','1lr-RW','PEIRS'};
AICs=[];
for i=1:length(models)
...
yticklabels(models_names)
xlabel('∆AICs','FontSize',8)
exportgraphics(f4,[figdir,'online_AICs.eps'],'BackgroundColor','none','ContentType','vector')
saveas(f4,[figdir,'online_AICs.png'])
...
%%
[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model2.pos_bias_lrs,'boottop','verbose','bootsamples',50000)
[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,1-model2.pos_bias_lrs,'boottop','verbose','bootsamples',50000)

[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,model2.pos_bias_lrs,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,'boottop','verbose','bootsamples',50000)

[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model1.mean_eta,'boottop','verbose','bootsamples',50000)


[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model2.pos_bias_lrs_logit,'boottop','verbose','bootsamples',50000)


[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ques_table.zscore_AD,model2.pos_bias_lrs,'boottop','verbose','bootsamples',50000)
[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ques_table.zscore_AD,1-model2.pos_bias_lrs,'boottop','verbose','bootsamples',50000)

[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ques_table.zscore_AD,model2.mean_alpha_neg./model2.mean_alpha_pos,'boottop','verbose','bootsamples',50000)

[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ques_table.zscore_AD,model1.mean_eta,'boottop','verbose','bootsamples',50000)


[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,model2.pos_bias_lrs,'boottop','verbose','bootsamples',50000)% This is only three that all correlate with each other but 0.05<Pab<0.1
[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,1-model2.pos_bias_lrs,'boottop','verbose','bootsamples',50000)% This is only three that all correlate with each other but 0.05<Pab<0.1
[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,model2.mean_alpha_neg_logit./model2.mean_alpha_pos_logit,'boottop','verbose','bootsamples',50000)% This is only three that all correlate with each other but 0.05<Pab<0.1

[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,model2.pos_bias_lrs_logit,'boottop','verbose','bootsamples',50000)

[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,model2.pos_bias_lrs(all_sub_include),'boot','verbose',10000000)
[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,model2.pos_bias_lrs(all_sub_include))

[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,model2.pos_bias_lrs,'boot','verbose',1000000)
[paths, stats]=mediation(ROI_GLM2.mPFC_chosen_NPE_all_mean,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,'boot','verbose',1000000)

[r,p]=corr(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model2.pos_bias_lrs)
[r,p]=corr(ROI_GLM2.bilateral_NAcc_chosen_PPE_all_mean,model2.pos_bias_lrs)
[r,p]=corr(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model2.mean_alpha_pos./model2.mean_alpha_neg)
[r,p]=corr(ques_table.zscore_AD,model2.mean_alpha_neg./model2.mean_alpha_pos)
[r,p]=corr(ques_table.zscore_AD,model2.pos_bias_lrs_logit)
[r,p]=corr(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model2.pos_bias_lrs)
[r,p]=corr(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model2.mean_alpha_neg./model2.mean_alpha_pos)

[paths, stats]=mediation(ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,ques_table.zscore_AD,model2.mean_alpha_pos-model2.mean_alpha_neg,'boottop','verbose','bootsamples',50000)% This is only three that all correlate with each other but 0.05<Pab<0.1
[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,model2.mean_alpha_pos-model2.mean_alpha_neg,'boottop','verbose','bootsamples',50000)


[paths, stats]=mediation(ROI_PPI_GLM2_sdbHb.bilateral_VTA_PPI_NPE_sd_bHb,ques_table.zscore_AD,ROI_GLM2.bilateral_NAcc_chosen_NPE_all_mean,'M',model2.pos_bias_lrs,'boot','verbose',100000)

%%
T=[ques_table,beh_results,model2,model1,struct2table(ROI_GLM2),struct2table(ROI_PPI_GLM2_sdbHb),struct2table(ROI_PPI_GLM2_sdVTA)];
